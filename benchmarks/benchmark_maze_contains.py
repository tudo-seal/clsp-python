...
def main(SIZE: int = 150, output: bool = False) -> float:
        ...
        def __contains__(self, value: object) -> bool:
            if isinstance(value, tuple):
                ...
                if isinstance(x, int) and isinstance(y, int):
                    if x >= 0 and x < SIZE and y >= 0 and y < SIZE and is_free((x, y)):
                        return True
            return False
            
    # "int2": list(filter(is_free, product(range(SIZE), range(SIZE))))
    literals = {"int2": Pos()}

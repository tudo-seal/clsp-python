"""Synthesizer implementing Finite Combinatory Logic with Predicates.
   It construct a logic program via `constructSolutionSpace` from the following ingredients:
...
from .grammar import Grammar, RHSRule, TerminalArgument, NonTermialArgument, NamedNonTermialArgument
                        ...
                        for nary_types in combinator_type:
                            ...
                            if not parameter_arguments: # do this only once for each instantiation
                                    parameter_arguments = tuple(TerminalArgument(n.name, instantiation[n.name].value)
                                                           if isinstance(n, Var) else NamedNonTermialArgument[Type](specific_params[n], n)
                            ...
                            for subquery in (
                                ...
                                memo.add_rule(
                                    current_target,
                                    RHSRule(
                                        parameter_arguments + tuple(NonTermialArgument(ty) for ty in subquery),
                                        term_predicates,
                                        combinator,
                                    ),
                                )

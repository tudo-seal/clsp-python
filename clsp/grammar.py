...
@dataclass(frozen=True)
class NonTermialArgument(Generic[NT]):
    value: NT

@dataclass(frozen=True)
class NamedNonTermialArgument(NonTermialArgument[NT]):
    name: str

Argument = TerminalArgument | NonTermialArgument[NT]
...
class RHSRule(Generic[NT, T]):
    ...
    def non_terminals(self) -> frozenset[NT]:
        ...
        return frozenset(arg.value for arg in self.arguments if isinstance(arg, NonTermialArgument))

    @property
    def argument_names(self) -> tuple[str | None, ...]:
        """Names of named arguments."""
        return tuple(a.name if isinstance(a, NamedNonTermialArgument) or isinstance(a, TerminalArgument) else None for a in self.arguments)
    ...
    def __str__(self) -> str:
        return "not implemented"  # TODO: implement __str__ for RHSRule
...
class Grammar(Generic[NT, T]):
    ...
    def _generate_new_trees(
        ...
        named_non_terminals = [a.value if isinstance(a, NamedNonTermialArgument) else None for a in rule.arguments]
        unnamed_non_terminals = [a.value if isinstance(a, NonTermialArgument) and not isinstance(a, NamedNonTermialArgument) else None for a in rule.arguments]
        ...
        specific_substitution = lambda parameters: {a.name: p for p, a in zip(parameters, rule.arguments) if isinstance(a, NamedNonTermialArgument)} | rule.literal_substitution

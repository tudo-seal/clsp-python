...
from .types import Type, Arrow, Intersection, Omega, Constructor
...
def show_grammar(grammar: dict[Clause, deque[tuple[object, list[Clause]]]]) -> Iterable[str]:
    for clause, possibilities in grammar.items():
        yield (show_clause(clause) + " => " + "; ".join((str(combinator)+"("+", ".join(map (show_clause, args))+")") for combinator, args in possibilities))
...
class Tree(object):
    ...
    def __str__(self):
        match self.rule:
            case Combinator(_, _): return str(self.rule.combinator)
...
class FiniteCombinatoryLogic(object):
    ...
    @staticmethod
    def _combinatory_expression_rules(combinator: object, arguments: list[Clause], target: Type) -> Iterable[Rule]:
    ...
    def _subqueries(self, nary_types: list[MultiArrow], paths: list[Type]) -> list[list[Type]]:
    ...
    def _combine_arguments(self, positive_arguments: list[list[Type]], negative_arguments: list[list[Type]]) -> list[list[Clause]]:
    ...
    def inhabit(self, *targets: Clause) -> InhabitationResult:
        ...
        while remaining_targets:
            ...
            if memo.get(target) is None:
                ...
                all_paths: tuple[list[Type], ...] = tuple(list(ty.organized) for ty in target)
                ...
                for combinator, combinator_type in self.repository.items():
                    for nary_types in combinator_type:
                        positive_arguments: list[list[Type]] = \
                            list(self._subqueries(nary_types, all_paths[0]))
                        negative_arguments: list[list[Type]] = \
                            list(chain.from_iterable(self._subqueries(nary_types, paths) for paths in all_paths[1:]))
                        for subquery in self._combine_arguments(positive_arguments, negative_arguments):
        ...
        for target, possibilities in memo.items():
            ...
            for combinator, arguments in possibilities:
                result.update(self._combinatory_expression_rules(combinator, arguments, FiniteCombinatoryLogic.clause_to_type(target)))

        #return InhabitationResult(targets=list(targets), rules=FiniteCombinatoryLogic._prune(result))
        return InhabitationResult(targets=list(map(FiniteCombinatoryLogic.clause_to_type, targets)), rules=result)

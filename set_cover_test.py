from cls_python.combinatorics import minimal_covers, maximal_elements
...
def equivalent_covers(covers1, covers2):
  return (len(covers1) == len(covers2) and
      all(any(equivalent_lists(c1, c2) for c2 in covers2) for c1 in covers1) and
      all(any(equivalent_lists(c1, c2) for c1 in covers1) for c2 in covers2))

print(naive_minimal_covers(sets, elements, contains))
print(equivalent_covers(minimal_covers(sets, elements, contains), naive_minimal_covers(sets, elements, contains)))
...
sets = [[], [5], [], [5, 5, 0, 0, 1, 4, 5, 4, 1], [5], [7, 7, 9, 3, 7, 0, 7, 7, 0, 3, 8, 4], [4, 6, 8, 7, 1, 9, 2, 8, 6], [7, 8, 9, 4, 1, 9, 3, 5], [8, 5]]
print(minimal_covers(sets, elements, contains))
print(naive_minimal_covers(sets, elements, contains))
print(equivalent_covers(minimal_covers(sets, elements, contains), naive_minimal_covers(sets, elements, contains)))
